...
pub fn register(commands: &mut Vec<CreateCommand>) {
    commands.push(
        CreateCommand::new("removeticket")
            .description("Remover usuario do ticket atual")
            .default_member_permissions(Permissions::MODERATE_MEMBERS)
            .add_option(
                CreateCommandOption::new(CommandOptionType::User, "user", "Usuario a remover")
                    .required(true),
            ),
    );
}
...
pub async fn handle(ctx: &Context, interaction: &CommandInteraction, _pool: &PgPool, _guild_cache: &GuildCache) -> Result<()> {
    ...
    ticket_manager::handle_remove_user_from_ticket(ctx, interaction, target).await
}
